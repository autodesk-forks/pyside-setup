...
properties([
  ...
  parameters([
	...
	string(name: 'QTBuildID', defaultValue: '33'),
  ])
])
...
def Setup(String buildConfig)
{
	...
	try {
		...
		if (checkOS() == "Mac") {
			...
			PysidePackage[buildConfig] = "${pysideVersion}-Maya-Pyside2-${env.BUILD_ID}-Qt-${params.QTVersion}-${params.QTBuildID}-win-v141.zip"
			artifacts[buildConfig]  = ["oss-stg-generic/Qt/${params.QTVersion}/Maya/${params.QTVersion}-Maya-Qt-${params.QTBuildID}-win-v141.zip", "team-maya-generic/libclang/release_70-based/libclang-release_70-based-windows-vs2015_64.zip", "team-asrd-pilots/openssl/102h/openssl-1.0.2h-win-vc14.zip"]
		}
