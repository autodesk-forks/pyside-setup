...
properties([
  ...
  parameters([
	...
	choice(name: 'PythonVersion', choices:['3.9.5'], description: 'Python version (format: A.B.C)'),
  ])
])
...
def Setup(String buildConfig) {
	...
	try {
		...
		if (checkOS() == "Mac") {
			...
			artifacts[buildConfig]  = ["${QtArtifact_Lnx}", "team-maya-generic/libclang/release_70-based/libclang-release_70-based-linux-Rhel7.2-gcc5.3-x86_64.tar.gz", "team-maya-generic/Cmake/cmake-3.22.1-linux-x86_64.tar.gz", "team-maya-generic/python/3.9.5/cpython-3.9.5-gcc-9.3.1-openssl-1.1.1k_manual_build-1.tar.gz"]
		}
		else {
